...
#define CHAIN_DLIGHT_TYPE_CODE (0x0011)
    ...
    if (devices_list) {
        for (uint8_t i = 0; i < devices_list->count; i++) {
            if (devices_list->devices[i].device_type == CHAIN_DLight_TYPE_CODE) {
                ...
                while (M5Chain.getDLightLuxTrigger(devices_list->devices[i].id, &dlight_lux_trigger_res, 100)) {
                    switch (dlight_lux_trigger_res) {
                        case CHAIN_DLIGHT_REPORT_LUX_HIGH:
                            Serial.printf("DLight ID[%d] lux trigger result is: high \r\n",
                                          devices_list->devices[i].id);
                            break;
                        case CHAIN_DLIGHT_REPORT_LUX_LOW:
                            Serial.printf("DLight ID[%d] lux trigger result is: low \r\n", devices_list->devices[i].id);
                            break;
                        default:
                            Serial.printf(
                                "DLight ID[%d] lux trigger result is: unknown , dlight_lux_trigger_res:%02x\r\n",
                                devices_list->devices[i].id, dlight_lux_trigger_res);
                            break;
                    }
                }
            }
