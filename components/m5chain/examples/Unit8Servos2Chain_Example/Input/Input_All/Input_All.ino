...
bool initializeInputModes()
{
    ...
    for (uint8_t i = 0; i < devices_list->count; i++) {
        ...
        chain_status = M5Chain.setServosModeAll(device_id, mode, GPIO_NUM_MAX, &operation_status);
        ...
        user_gpio_mode_t read_mode[GPIO_NUM_MAX];
        if (M5Chain.getServosModeAll(device_id, read_mode, GPIO_NUM_MAX) != CHAIN_OK) {
        ...
        chain_status = M5Chain.setServosInputPuPdAll(device_id, pull, GPIO_NUM_MAX, &operation_status);
        ...
        user_gpio_pull_t read_pull[GPIO_NUM_MAX];
        if (M5Chain.getServosInputPuPdAll(device_id, read_pull, GPIO_NUM_MAX) != CHAIN_OK) {
...
bool readAndDisplayInputLevels()
{
    ...
    for (uint8_t i = 0; i < devices_list->count; i++) {
        ...
        user_gpio_mode_t mode[GPIO_NUM_MAX];
        user_gpio_pull_t pull[GPIO_NUM_MAX];
        user_sys_gpio_level_t state[GPIO_NUM_MAX];

        // Step 1: Read and verify current mode / 步骤1：读取并验证当前模式
        if (M5Chain.getServosModeAll(device_id, mode, GPIO_NUM_MAX) != CHAIN_OK) {
            Serial.printf(
        ...
        if (M5Chain.getServosInputPuPdAll(device_id, pull, GPIO_NUM_MAX) != CHAIN_OK) {
            Serial.printf(
        ...
        if (M5Chain.getServosInputLevelAll(device_id, state, GPIO_NUM_MAX) != CHAIN_OK) {
            Serial.printf(
