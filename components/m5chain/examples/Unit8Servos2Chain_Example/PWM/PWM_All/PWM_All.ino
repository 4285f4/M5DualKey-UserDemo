...
uint8_t duty[GPIO_NUM_MAX] = {10, 20, 30, 40, 50, 60, 70, 80}; /**< PWM duty cycles in percentage / PWM占空比(百分比) */
...
bool initializePwmModes()
{
    ...
    for (uint8_t i = 0; i < devices_list->count; i++) {
        ...
        chain_status = M5Chain.setServosPwmDutyAll(device_id, duty, GPIO_NUM_MAX, &operation_status);
        ...
        uint8_t read_duty[GPIO_NUM_MAX];
        if (M5Chain.getServosPwmDutyAll(device_id, read_duty, GPIO_NUM_MAX) != CHAIN_OK) {
...
bool verifyPwmConfiguration()
{
    ...
    for (uint8_t i = 0; i < devices_list->count; i++) {
        ...
        uint8_t duty_temp[GPIO_NUM_MAX];
        ...
        if (M5Chain.getServosPwmDutyAll(device_id, duty_temp, GPIO_NUM_MAX) != CHAIN_OK) {
            Serial.printf(
