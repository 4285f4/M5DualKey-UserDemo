...
bool validateAndSetServoAngles()
{
    ...
    for (uint8_t i = 0; i < devices_list->count; i++) {
        ...
        uint8_t angle_test[GPIO_NUM_MAX];
        for (uint8_t j = 0; j < GPIO_NUM_MAX; j++) {
            angle_test[j] = angle;
        }

        // Step 3: Write angle values / 步骤3：写入角度值
        chain_status = M5Chain.setServosAngleAll(device_id, angle_test, GPIO_NUM_MAX, &operation_status);
        ...
        uint8_t angle_get[GPIO_NUM_MAX] = {0};
        if (M5Chain.getServosAngleAll(device_id, angle_get, GPIO_NUM_MAX) != CHAIN_OK) {
