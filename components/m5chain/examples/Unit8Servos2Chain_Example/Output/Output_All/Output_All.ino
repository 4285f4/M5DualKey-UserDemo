...
bool toggleOutputLevels()
{
    ...
    for (uint8_t i = 0; i < devices_list->count; i++) {
        ...
        uint8_t device_id = devices_list->devices[i].id;
        user_gpio_mode_t mode[GPIO_NUM_MAX];
        user_sys_gpio_level_t level[GPIO_NUM_MAX];
        ...
        if (M5Chain.getServosOutputLevelAll(device_id, level, GPIO_NUM_MAX) != CHAIN_OK) {
            Serial.printf(
        ...
        user_sys_gpio_level_t new_level[GPIO_NUM_MAX];
        for (uint8_t j = 0; j < GPIO_NUM_MAX; j++) {
            new_level[j] = (user_sys_gpio_level_t)(1 - level[j]);
        }

        // Step 4: Write new output levels / 步骤4：写入新的输出电平
        chain_status = M5Chain.setServosOutputLevelAll(device_id, new_level, GPIO_NUM_MAX, &operation_status);
        ...
        user_sys_gpio_level_t verify_level[GPIO_NUM_MAX];
        if (M5Chain.getServosOutputLevelAll(device_id, verify_level, GPIO_NUM_MAX) != CHAIN_OK) {
            Serial.printf(
