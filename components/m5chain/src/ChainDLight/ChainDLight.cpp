...
{
    return CHAIN_DLIGHT_DEVICE_TYPE_CODE;
}

bool ChainDLight::getDLightLuxTrigger(uint8_t id, chain_dlight_event_trigger_t *triggerStatus, unsigned long timeout)
{
    ...
    record_node_t *current  = recordList.head;
    record_node_t *previous = NULL;

    while (current) {
        if (current->info.id == id &&
            (current->info.type == CHAIN_DLIGHT_REPORT_LUX_LOW ||
             current->info.type == CHAIN_DLIGHT_REPORT_LUX_HIGH)) {
            *triggerStatus = (chain_dlight_event_trigger_t)current->info.type;
            if (previous) {
