...
/**
 * @brief CHAIN_DLIGHT device type code.
 */
#define CHAIN_DLIGHT_DEVICE_TYPE_CODE (0x0011)
...
typedef enum {
    CHAIN_DLIGHT_REPORT_LUX_LOW  = 0x1100, /**< Interrupt event triggered when lux value goes out of low threshold. */
    CHAIN_DLIGHT_REPORT_LUX_HIGH = 0x1101, /**< Interrupt event triggered when lux value goes out of high threshold. */
...
typedef enum {
    ...
    CHAIN_DLIGHT_RATE_100MS  = 0x02, /**< 100 ms, default. */
...
public:
     ...
     *
     * The measurement rate should be greater than the conversion time required by the configured resolution.
     *
     * @param id The position of the DLight device in the chain (starting from 1).
    ...
    /**
     * @brief Gets the Chain DLight lux trigger status.
     *
     * @param id The position of the DLight device in the chain (starting from 1).
     * @param triggerStatus Pointer to store the trigger status.
     * @param timeout The timeout duration for the operation in milliseconds (default is 100ms).
     *
     * @return Operation status.
     */
    bool getDLightLuxTrigger(uint8_t id, chain_dlight_event_trigger_t *triggerStatus, unsigned long timeout = 100);
