 ...
 *
 *   - 快速点按（物理按住约 100ms）若被量成约 1000ms → 松手边沿被晚检出；
 *   - 若被量成约 100ms → 边沿及时，问题在报告送达（USB 栈 / 多通道竞争）。
 *
 * ⚠️ 盲点（2026-09-15 实测后补）：若 down/up 两个边沿被**整体平移**同一时间
 *    （各自都晚检出 1s），差值不变 ⇒ 本指标测不出。故"tap 正常"只排除"单个边沿
 *    被拖慢"，不能单独排除"整体平移延迟"。
 *
 * ⚠️ `cb_gap_ms_max` 名不副实：`btn_progress()` 是**按键事件回调**（只在 down/up
 *    跳变时被调，不是每 1ms 扫描都调 —— 证据：cb_down_count == tap_count+long_count），
 *    所以它算出的"相邻回调间隔"实际等于**一次按住的持续时长（up - down）**，
 *    与 tap_hold_ms 同源，**测不出任务被饿死**。
 *
